...
TCM::TCM(int depth, int length, int width, int lgn) {
...
void TCM::Update(uint32_t * key, val_tp val) {
    mv_.sum += val;
    ...
    for (int i = 0; i < mv_.depth; i++) {
        ...
        TCM::SBucket *sbucket = mv_.counts[index];
        sbucket->sum += val;
//        if (sbucket->key[0] == '\0') {
...
val_tp TCM::Queryedgeweight(uint32_t* key) {
    val_tp ret = 0;
...
val_tp TCM::Querynodeweight(uint32_t* key) {
    val_tp ret = 0;
    for (int j = 0; j < mv_.depth; j++) {
        unsigned long row = MurmurHash64A(key, mv_.lgn/2, mv_.hardner[j]) % mv_.width;
        val_tp value = 0;
...
//void TCM::Queryheavyhitteredge(val_tp thresh, std::vector<std::pair<key_tp, val_tp> >&results) {
//
//    myset res;
...
//}
//
//void TCM::Queryheavyhitternode(val_tp thresh, std::vector<std::pair<key_tp, val_tp> >&resultsnode) {
//    myset res;
...
bool TCM::BFS(uint32_t* key, std::vector<std::pair<key_tp, val_tp> >& path){
    myset res, dstres;
    key_tp reskey;
...
val_tp TCM::Change(TCM** mv_arr){
    TCM* oldsk = mv_arr[0];
    TCM* cursk = mv_arr[1];
    int depth = cursk->mv_.depth, length = cursk->mv_.length, width = cursk->mv_.width;
    TCM::SBucket** oldtable = (oldsk)->GetTable();
    TCM::SBucket** curtable = (cursk)->GetTable();
...
//void TCM::Queryheavychangeredge(val_tp thresh, std::vector<std::pair<key_tp, val_tp> >&results) {
//
//    myset res;
//    for (int i = 0; i < mv_.length*mv_.width*mv_.depth; i++) {
//        if (mv_.counts[i]->sum > thresh) {
...
//
//
//void TCM::Queryheavychangernode(val_tp thresh, std::vector<std::pair<key_tp, val_tp> >&results) {
//
//    myset keyset;
//    for (int i = 0; i < mv_.length*mv_.width*mv_.depth; i++) {
//        key_tp reskey;
...
//}
//
//val_tp TCM::Low_estimate(uint32_t* key) {
//    val_tp ret = 0;
//    for (int j = 0; j < mv_.depth; j++) {
//        unsigned long row = MurmurHash64A(key, mv_.lgn/2, mv_.hardner[j]) % mv_.width;
//        unsigned long col = MurmurHash64A(key+1, mv_.lgn/2, mv_.hardner[j]) % mv_.length;
...
//
//
//val_tp TCM::Low_estimatenode(uint32_t* key) {
//    val_tp ret = 0;
//    for (int j = 0; j < mv_.depth; j++) {
//        unsigned long row = MurmurHash64A(key, mv_.lgn/2, mv_.hardner[j]) % mv_.width;
//        val_tp value = 0;
...
//void TCM::FindSuper(mymap& super){
