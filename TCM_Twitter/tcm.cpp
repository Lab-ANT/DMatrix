...
int main(int argc, char* argv[]) {
    rr::RrConfig config;
    if(config.ReadConfig("config_Twitter.ini") == false) {
	std::cout << "Error loading configuration file." << std::endl;
	return 1;
    }
    int num = 1;
    for(;;) {
	std::string dm_config = "TCM_" + std::to_string(num);
	...
	int nodenum = config.ReadInt(dm_config.c_str(), "nodenum", 0);
        ...
        //TCM
        HeavyChanger<TCM>* heavychangermv = new HeavyChanger<TCM>(mv_depth, mv_length, mv_width, LGN);
    ...
    //            }
            ...
            TCM* cursk = (TCM*)heavychangermv->GetCurSketch();
            TCM* oldsk = (TCM*)heavychangermv->GetOldSketch();
            t1 = now_us();
            while(adaptor->GetNext(&t) == 1) {
                key_tp key;
                memcpy(key.key, &(t.key), LGN);
            ...
            for (auto it = groundedgetmp.begin(); it != groundedgetmp.end(); it++) {
                if (it->second[0] > 0) {
                    val_tp edgeweight;
                    edgeweight = cursk->Queryedgeweight((uint32_t*)(*it).first.key);
                    if (edgeweight > thresholdhitter){
                    key_tp key;
                    memcpy(key.key, (*it).first.key, LGN);
                    std::pair<key_tp, val_tp> node;
                    node.first = key;
                    node.second = edgeweight;
                    heavyhitteredge.push_back(node);
                    }
                }
            }
            t2 = now_us();
            ...
            for (auto it = groundnodetmp.begin(); it != groundnodetmp.end(); it++) {
                if (it->second[0] > 0) {
                    val_tp nodeweight;
                    nodeweight = cursk->Querynodeweight((uint32_t*)(*it).first.key);
                    if (nodeweight > thresholdhitter){
                    key_tp key;
                    memcpy(key.key, (*it).first.key, LGN/2);
                    std::pair<key_tp, val_tp> node;
                    node.first = key;
                    node.second = nodeweight;
                    heavyhitternode.push_back(node);
                    }
                }
            }
            t2 = now_us();
    ...
    //        }
            ...
            if (numfile != 0) {
                ...
                t1 = now_us();
                for (auto it = groundedgetmp.begin(); it != groundedgetmp.end(); it++) {
                val_tp oldval, newval, change;
                oldval = oldsk->Queryedgeweight((uint32_t*)(*it).first.key);
                newval = cursk->Queryedgeweight((uint32_t*)(*it).first.key);
                change = newval > oldval ? newval - oldval : oldval - newval;
                if (change > thresholdchanger){
                    key_tp key;
                    memcpy(key.key, (*it).first.key, LGN);
                    std::pair<key_tp, val_tp> node;
                    node.first = key;
                    node.second = change;
                    heavychangeredge.push_back(node);
                }
                }
                t2 = now_us();
                ...
                if (heavychangeredge.size() == 0){ prechangeredge = 1; }
                else {prechangeredge = tp*1.0/heavychangeredge.size();}
                if (groundedge.size()==0){rechangeredge = 1;}
                else { rechangeredge = tp*1.0/groundedge.size();}
                if (tp == 0) { reerchangeredge = 0;}
                else {reerchangeredge = reerchangeredge/tp;}
                ...
                for (auto it = groundnodetmp.begin(); it != groundnodetmp.end(); it++) {
                val_tp oldval, newval, change;
                oldval = oldsk->Querynodeweight((uint32_t*)(*it).first.key);
                newval = cursk->Querynodeweight((uint32_t*)(*it).first.key);
                change = newval > oldval ? newval - oldval : oldval - newval;
                if (change > thresholdchanger){
                    key_tp key;
                    memcpy(key.key, (*it).first.key, LGN/2);
                    ...
                    node.first = key;
                    node.second = change;
                    heavychangernode.push_back(node);
                }
                }
                t2 = now_us();
    ...
    //            }
                ...
                if (heavychangernode.size()==0){ prechangernode = 1;}
                else {prechangernode = tp*1.0/heavychangernode.size();}
                if (groundnode.size()==0) { rechangernode = 1;}
                else {rechangernode = tp*1.0/groundnode.size();}
                if (tp==0) { reerchangernode = 0; }
                else { reerchangernode = reerchangernode/tp;}
