...
Adaptor::Adaptor(std::string filename, uint64_t buffersize) {
	...
	int pos2 = srcdst.find("\t",pos1+1);
        int pos3 = srcdst.find("\r\n");
        std::string ip1 = srcdst.substr(0,pos1);
        std::string ip2 = srcdst.substr(pos1+1,pos2-pos1-1);
	std::string fnum = srcdst.substr(pos2+1,pos3-pos2-1);
        int srcip = atoi(ip1.c_str());
        int dstip = atoi(ip2.c_str());
	int frequency = atoi(fnum.c_str());
	...
	memcpy(p, &srcip, sizeof(uint32_t));
        memcpy(p+sizeof(uint32_t), &dstip, sizeof(uint32_t));
        memcpy(p+sizeof(uint32_t)*2, &frequency, sizeof(uint32_t));
        p += sizeof(uint32_t)*2+sizeof(uint32_t);
        data->cnt++;
    }
    infile.close();
}
